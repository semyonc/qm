...
{
    public abstract class XQueryWriter
    {
        ...
        public virtual void WriteFLWORExpr(Notation.Record rec)
        {
        ...
        {
            if (notation.Flag(rec.sym, Descriptor.Parallel))
                WriteText("(:+ parallel +:) ");
            WriteText("for ");
            Symbol[] arr = Lisp.ToArray<Symbol>(rec.args[0]);
            for (int k = 0; k < arr.Length; k++)
        ...
        public virtual void WriteWhereClause(Symbol sym)
        {
            ...
            {
                WriteText(" where ");
                bool? enableHashJoin = notation.GetFlag(sym, Descriptor.HashJoin);
                if (enableHashJoin.HasValue)
                    WriteTextFormat("(:+ enable_hash_join := {0} +:) ", enableHashJoin.Value);
        ...
        protected abstract void SmartNewLine();

        protected abstract void WriteText(char c);

        protected abstract void WriteText(string s);
    ...
    public class XQueryTextWriter : XQueryWriter
    {

        private StringBuilder sb;
        private bool newLineFlag;

        public XQueryTextWriter(Notation notation)
            : base(notation)
        {
    ...
    public class XQueryNullWriter : XQueryWriter
    {
        public XQueryNullWriter(Notation notation)
            : base(notation)
        {
        }
    ...
    public class XQueryTokenWriter : XQueryWriter
    {

        private List<Object> tokens;
        private Stack<int> anchor;

        public XQueryTokenWriter(Notation notation)
            : base(notation)
        {
            tokens = new List<object>();
            anchor = new Stack<int>();
        }
        ...
        public override void WriteQName(Qname n)
        {
            tokens.Add(n);
        }

        public override void WriteDecimal(DecimalValue decimalValue)
        {
            tokens.Add(decimalValue);
        }

        public override void WriteDouble(DoublelValue doubleValue)
        {
            tokens.Add(doubleValue);
        }

        public override void WriteInteger(IntegerValue integerValue)
        {
            tokens.Add(integerValue);
        }

        public override void WriteLiteral(Literal lit)
        {
            tokens.Add(lit);
        }

        public override void WriteVarName(VarName n)
        {
            tokens.Add(n);
        }
        #endregion
        ...
        {
            tokens.CopyTo(index, array, arrayIndex, count);
        }

        public bool Match(params object[] template)
        {
            return Match(template, null);
        }

        public bool Match(object[] template, List<Object> res)
        {
            if (template.Length <= tokens.Count)
            {
                for (int k = 0; k < template.Length; k++)
                {
                    object t = template[template.Length - 1 - k];
                    if (t == null)
                    {
                        if (res != null)
                            res.Add(tokens[Count - k - 1]);
                    }
                    else
                        if (!t.Equals(tokens[Count - k - 1]))
                            return false;
                }
                return true;
            }
