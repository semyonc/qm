...
namespace DataEngine
{
    class DetailJoin: QueryNode
    {
        ...
        public override Resultset Get(QueryContext queryContext, object[] parameters)
        {
            ...
            src1.Clear();
            HashSet<RowType.TypeInfo> fields_set =
                new HashSet<RowType.TypeInfo>(new FieldComparer());
            foreach (JoinNode node in nodes)
                foreach (RowType.TypeInfo nested_ti in node.type.Fields)
                    if (!nested_ti.IsHidden && !fields_set.Contains(nested_ti))
                        fields_set.Add(nested_ti);
            ...
            int k = 0;
            foreach (RowType.TypeInfo ti in fields_set)
            {
            ...
            context.Iterator = NextRow(rs, nodes, nested_type, map);
            return rs;
        }

        private IEnumerator<Row> NextRow(Resultset rs, List<JoinNode> nodes, 
