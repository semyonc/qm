...
namespace DataEngine.CoreServices.Generation {
    ...
    public class ILGen {
        ...
        public void EmitPropertyGet(PropertyInfo pi) {
            if (!pi.CanRead) {
                throw new InvalidOperationException("can't read from property");
            }
        ...
        public void EmitPropertySet(PropertyInfo pi) {
            if (!pi.CanWrite) {
                throw new InvalidOperationException("can't write to property");
            }
        ...
        public void EmitNew(ConstructorInfo ci) {
            if (ci.DeclaringType.ContainsGenericParameters) {
                throw new ArgumentException(String.Format("Cannot create instance of {0} because it contains generic parameters", ci.DeclaringType)); 
            }
        ...
        public void EmitSimpleConstant(object value) {
        ...
        private void EmitCastToType(Type typeFrom, Type typeTo) {
            if (!typeFrom.IsValueType && typeTo.IsValueType) {
                ...
                throw new InvalidOperationException(
                    String.Format("Cannot cast from type '{0}' to type '{1}", typeFrom, typeTo));
            }
        }
                    ...
                    if (isFromUnsigned) {
                        switch (tc) {
                            ...
                            case TypeCode.UInt64:
                                ...
                                break;
                            default:
                                throw new ArgumentException(String.Format("Unhandled convert: {0}", typeTo));
                        }
                    } else {
                        switch (tc) {
                            case TypeCode.SByte:
